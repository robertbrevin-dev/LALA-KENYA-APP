...
export default function HostListings() {
  ...
  const handleCreate = async () => {
    ...
    if (!title || !area || !price || files.length === 0) {
      setError('Title, area, price and at least one photo are required');
      return;
    }
    setSaving(true);
    setError('');
    try {
      ...
      for (let i = 0; i < Math.min(files.length, 6); i++) {
        ...
        const path = `properties/${user.id}/${Date.now()}-${i}.${ext}`;
        const { error: upErr } = await supabase.storage.from('property-images').upload(path, f, { upsert: true });
        if (upErr) {
          const b = await f.arrayBuffer();
          const blob = new Blob([b], { type: f.type || 'image/jpeg' });
          const fb = new File([blob], `fallback-${Date.now()}-${i}.jpg`, { type: 'image/jpeg' });
          const path2 = `properties/${user.id}/${Date.now()}-fb-${i}.jpg`;
          const { error: upErr2 } = await supabase.storage.from('property-images').upload(path2, fb, { upsert: true });
          if (!upErr2) {
            const { data } = supabase.storage.from('property-images').getPublicUrl(path2);
      ...
      const rules: string[] = [partyAllowed ? 'Parties allowed' : 'No parties'];
