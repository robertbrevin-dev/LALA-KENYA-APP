...
export default function HostForgotPassword() {
  ...
  const handleReset = async () => {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${window.location.origin}/reset-password?role=host`,
    });
    ...
    return (
      <div className="flex items-center justify-center min-h-screen" style={{ background: '#03020a' }}>
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
          ...
          style={{ background: 'linear-gradient(170deg, #06100e, #080608)', border: '1px solid rgba(62,207,178,0.1)', boxShadow: '0 60px 120px rgba(0,0,0,0.9)' }}>
          ...
          <p className="text-[12px] text-center mb-8" style={{ color: 'rgba(255,255,255,0.35)', lineHeight: 1.7 }}>
            Click the link in your email to reset your host account password. Check your spam folder if you don't see it.
          </p>
          <motion.button whileTap={{ scale: 0.97 }} onClick={() => navigate('/login/host')}
            ...
            style={{ background: `linear-gradient(135deg, ${TEAL}, #2AA893)`, color: '#06100e' }}>
            Back to Host Login
          </motion.button>
        </motion.div>
      </div>
  ...
  return (
    <div className="flex items-center justify-center min-h-screen" style={{ background: '#03020a' }}>
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }}
        ...
        style={{ background: 'linear-gradient(170deg, #06100e, #080608)', border: '1px solid rgba(62,207,178,0.1)', boxShadow: '0 60px 120px rgba(0,0,0,0.9)' }}>
        <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 80% 60% at 50% 20%, rgba(62,207,178,0.06) 0%, transparent 70%)' }} />
        <div className="relative z-10 flex flex-col flex-1 px-8 pt-16 pb-10">
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => navigate('/login/host')}
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
            ...
            <p className="text-[13px]" style={{ color: 'rgba(255,255,255,0.4)', lineHeight: 1.7 }}>Enter your host account email and we'll send you a reset link.</p>
          </motion.div>
          <div className="mb-6">
            <label className="text-[11px] mb-2 block font-bold" style={{ color: 'rgba(255,255,255,0.4)', letterSpacing: 1 }}>HOST EMAIL ADDRESS</label>
          ...
          <motion.button whileTap={{ scale: 0.97 }} onClick={handleReset} disabled={loading}
            className="w-full py-4 rounded-[16px] border-none cursor-pointer font-bold text-[15px]"
            style={{ background: loading ? 'rgba(255,255,255,0.1)' : `linear-gradient(135deg, ${TEAL}, #2AA893)`, color: loading ? 'rgba(255,255,255,0.4)' : '#06100e' }}>
