...
export default function HostProfile() {
  ...
  const menuItems = [
    { icon: '🏠', label: 'My Listings', path: '/host/listings' },
    { icon: '💰', label: 'Payout Methods', path: '/host/settings/payouts' },
    { icon: '📊', label: 'Performance Insights', path: '/host/settings/insights' },
    { icon: '📅', label: 'All Bookings', path: '/host/bookings' },
    { icon: '💵', label: 'Earnings', path: '/host/earnings' },
    { icon: '❓', label: 'Host Resources', path: '/host/settings/resources' },
    { icon: '⚙️', label: 'Account Settings', path: '/host/settings/account' },
  ];
        ...
        <div className="px-6 pt-14 pb-8 relative overflow-hidden"
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative z-10">
            ...
            <div className="text-center">
              ...
              <div className="text-[13px] mb-1" style={{ color: 'rgba(13,15,20,0.6)', fontWeight: 500 }}>
                {currentUser?.email}
              </div>
              <div className="text-[12px]" style={{ color: 'rgba(13,15,20,0.5)', fontWeight: 500 }}>
                Host · {currentUser?.phone}
              </div>
            </div>
          </motion.div>
        </div>
