  ...
  return (
    <PhoneFrame>
      ...
      <div className="relative flex-1 overflow-y-auto" style={{ scrollbarWidth: 'none', zIndex: 1 }}>
        ...
        {/* HERO */}
        <div className="h-[240px] relative flex items-center justify-center overflow-hidden"
          style={{ background: 'linear-gradient(135deg, #1a1208 0%, #0e0b06 50%, #080608 100%)' }}>
          <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse 70% 60% at 50% 50%, rgba(232,184,109,0.15) 0%, transparent 70%)' }} />
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.6 }}
            className="text-[80px] relative z-10" style={{ filter: 'drop-shadow(0 20px 40px rgba(232,184,109,0.3))' }}>
            {property.image}
          </motion.div>
          <div className="absolute bottom-0 left-0 right-0 h-24" style={{ background: 'linear-gradient(transparent, #080608)' }} />
          <motion.button initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
            onClick={() => navigate(-1)}
            className="absolute top-14 left-5 w-9 h-9 rounded-[12px] flex items-center justify-center border-none cursor-pointer z-10"
            style={{ background: 'rgba(8,6,8,0.8)', backdropFilter: 'blur(16px)', border: '1px solid rgba(232,184,109,0.2)', color: 'white', fontSize: 16 }}>
            ←
          </motion.button>
          <motion.button initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
            className="absolute top-14 right-5 w-9 h-9 rounded-[12px] flex items-center justify-center border-none cursor-pointer z-10"
            style={{ background: 'rgba(8,6,8,0.8)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.1)', fontSize: 16 }}>
            🤍
          </motion.button>
        </div>
