    ...
    <AnimatePresence>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ color: 'rgba(62,207,178,0.9)', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => rejectCall(incomingCall)}
            style={{ width: 42, height: 42, borderRadius: '50%', background: '#dc2626', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 12px rgba(220,38,38,0.5)' }}>
            <PhoneOff size={18} color="white" />
          </button>
          <button onClick={() => acceptCall(incomingCall)}
            style={{ width: 42, height: 42, borderRadius: '50%', background: '#16a34a', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 12px rgba(22,163,74,0.5)' }}>
...
with open('src/app/components/IncomingCallBanner.tsx', 'w', encoding='utf-8') as f:
    f.write(banner)
print("Banner updated with fixed positioning")
